...
{
    DeviceLocal  = 1 << 0,  // GPU-only, CPU not visible
    HostVisible  = 1 << 1,  // CPU-writable (staging)
    HostCoherent = 1 << 2,  // No flush needed (GL ignores)
    HostCached   = 1 << 3,  // CPU readback optimized (GL ignores)
};
...
enum class LoadOp : uint32_t
{
    ...
    Clear,      // Clear with clearValue
    DontCare,   // Content undefined (best performance)
};

enum class StoreOp : uint32_t
{
    Store,      // Preserve for later use
    DontCare,   // Discard (e.g. depth after pass)
};
