...
//
// Maps to:
//   Vulkan: VkFormat (VK_FORMAT_R8_UNORM, etc.)
//   OpenGL: GLenum (GL_R8, GL_RGBA8, etc.)
//   DirectX: DXGI_FORMAT
...
enum class Format : uint32_t
{
    ...
    // ---- 32-bit Integer (atomically writable, used for ID buffers) ----
    R32_UINT,
    R32_SINT,
    RG32_UINT,
    RGBA32_UINT,
    ...
    // ---- BC Compressed (block-compressed textures) ----
    BC1_RGB_UNORM,
    BC3_RGBA_UNORM,
    BC5_RG_UNORM,
    BC7_RGBA_UNORM,
...
// Returns the byte size of a single pixel / texel block.
// For block-compressed formats, returns the block size in bytes.
...
// Returns true if the format contains a depth component.
...
// Returns true if the format contains a stencil component.
