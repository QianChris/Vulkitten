...
目前已实现 OpenGL 4.6 和 Vulkan 1.3 双后端，通过 `rhi::BackendType` 一行切换。VulkittenRHI 完成 DLL 化（`RHI_API` 符号导出），依赖项统一迁移至 `3rdparty/` 目录。
...
| 目标 | 类型 | 说明 |
|------|------|------|
| `VulkittenRHI` | 动态库 (DLL) | 渲染硬件接口：`rhi` 命名空间下的设备/命令缓冲/资源抽象 + GL/VK 后端。GL 后端使用原生 WGL（零 GLFW 依赖）。导出 12 个 `RHI_API` 类。 |
| `Vulkitten` | 动态库 (DLL) | 引擎核心：ECS、渲染器抽象、ImGui、事件系统（依赖于 VulkittenRHI） |
| `Sandbox` | 可执行文件 | 引擎测试应用 |
| `RHISample` | 可执行文件 | RHI 示例程序，单文件展示 OpenGL/Vulkan 后端切换 |
| `VulkittenEditor` | 可执行文件 | 编辑器：场景层级、属性面板、视口(Gizmo+拾取)、性能面板、资源面板 |
| `3rdparty/` | 依赖目录 | 所有第三方库（git submodules + 手动），项目间共享 |
| **Tests** | | |
| `VulkittenRHIUnitTests` | 可执行文件 | 68 个 Mock 单元测试（无 GPU） |
| `VulkittenRHIGLTests` | 可执行文件 | 18 个 OpenGL 集成测试（需 GPU） |
| `VulkittenRHIVKTests` | 可执行文件 | 14 个 Vulkan 集成测试（需 GPU + Vulkan SDK） |
...
### ✅ Phase: Renderer 深度重构 (Tasks 1-8, 2026-06-20)

全部 8 个任务完成，覆盖四大支柱：

1. **平台层抽象** — IWindow（后端窗口接口）+ ISurface（平台绘制表面）+ WindowsSurface（GLFW 实现）。WindowsWindow 多重继承 Window + IWindow。
2. **后端统一接口** — IRenderer 作为平台/场景层接触的唯一后端接口，定义 BeginFrame/EndFrame/Execute/OnWindowResize 生命周期。Device→IDevice 重命名。IGpuResourceManager 抽象 GPU 资源创建（CreateTexture/CreateBuffer/CreateShader/CreatePipeline/CreateGeometry）。
3. **每帧架构** — FrameContext（FrameIndex/SwapchainIndex/CommandPool/Fence/Semaphore）为 Vulkan 预留。Per-Pass RenderContext 翻译 RenderCommand→API Drawcall。旧 RenderContext 重命名为 RendererSubsystem。
4. **依赖注入** — SceneContext（IRenderer& + RenderGraph&）通过 Layer::OnUpdate(Timestep, SceneContext&) 注入，Scene/System 不再依赖全局单例。Application 主循环使用 IRenderer::BeginFrame→Update→Execute→EndFrame 生命周期。SwapBuffers 从 EndPass 迁移至 IRenderer::EndFrame()。

### ✅ Phase: 资源管线现代化 (Tasks 1-7, 2026-06-21)

全部 7 个任务完成：

1. **IRenderer::Get() 崩溃修复** — s_Current 移至 Init() 开头
2. **Renderer.h 平台依赖清理** — Scope<IDevice>/Scope<IGpuResourceManager> 替代具体类型
3. **ShaderManager→IGpuResourceManager** — 统一所有 GPU 资源（Buffer/Texture/Shader）到同一接口，独立 ShaderManager 类已删除
4. **SPIR-V 直接加载** — OpenGLShader 使用 glShaderBinary+glSpecializeShader 从 .spv 创建 Program，移除 ~200 行 GLSL #include 预处理
5. **路径迁移** — 所有 .shader JSON 引用替换为无扩展名 SPIR-V 路径
6. **编译脚本** — compile_shader.py 支持 --target/--watch/--include-dirs，新增 compile_shader_vulkan.bat
7. **E2E 验证** — Sandbox 使用 OpenGL 后端，3 Quad 渲染 + Resize 编译通过
...
### ✅ glTF 加载支持 (2026-06-20)

- 添加 tinygltf 作为 git submodule（`Vulkitten/vendor/tinygltf/`），header-only 模式编译
- 创建 GltfLoader 模块（`Vulkitten/Renderer/Gltf/GltfLoader`）封装模型加载、顶点提取
- 新增 ExampleLayer3D 演示：3D 模型可视化 + 简单光照着色器 + 轨道相机
- 引擎 Shader 资产新增 Simple3D.vert/.frag/.shader
