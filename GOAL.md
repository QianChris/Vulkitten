...
- **双渲染路径仍共存**：Scene 中有 System 时走 RenderGraph 路径（✅），无 System 时走旧 RenderScene→Renderer2D 回退路径。Task 3 将移除旧路径
- **EditorLayer 的 Framebuffer 尚未迁移到 Pass 配置**：当前仍直接调用 Legacy::RenderCommand::Clear。Task 2 将处理
