# RHI (Rendering Hardware Interface) 设计文档 v1.2
...
graph TB
    ...
    subgraph RHI["RHI Abstraction"]
        ...
        RCL["RenderCommandList<br/>薄适配层 / 高层便捷 / 设备访问"]
    ...
    RCL -->|代理| ICommandBuffer
    RCL -->|引用| IRenderDevice
    RCL -->|引用| ResourceManager
...
classDiagram
    ...
    class RenderCommandList {
        ...
        +bindPipeline(pso) RenderCommandList
        +bindGeometry(geo) RenderCommandList
        +bindUniformBuffer(slot, buf, off, range) RenderCommandList
        +bindTexture(slot, tex, smp) RenderCommandList
        +pushConstants(data, offset) RenderCommandList
        +drawIndexed(count, first, base) RenderCommandList
        +dispatch(gx, gy, gz) RenderCommandList
        +barrier(tex, srcS, srcA, dstS, dstA, oldL, newL) RenderCommandList
        +getFrameIndex() uint32_t
    ...
    RenderCommandList --> ICommandBuffer : 持有并代理
    RenderCommandList --> IRenderDevice : 持有引用
    RenderCommandList --> ResourceManager : 持有引用
...
sequenceDiagram
    ...
    Ren->>RCL: new RenderCommandList(device, rm, cmd)
    Ren-->>App: FrameContext

    App->>RG: execute(list)
    RG->>Cmd: begin()
    RG->>Cmd: beginRenderPass(...)

    loop Each RenderPass
        RG->>Pass: pass.execute(list)

        Note over Pass,RCL: Pass 调用 RenderCommandList<br/>List 代理到 ICommandBuffer
        ...
        alt 风格A: 高层便捷
            ...
            Pass->>RCL: list.bindPipeline(pso).bindGeometry(geo).drawIndexed(6,0,0)
            RCL->>Cmd: bindPipeline(pso)
            RCL->>Cmd: bindGeometry(geo)
            RCL->>Cmd: drawIndexed(6, 0, 0)
        else 风格C: 直接操作底层
            Pass->>RCL: list.cmd().beginDebugLabel("MyPass")
            Pass->>RCL: list.cmd().bindPipeline(pso)
            Pass->>RCL: list.cmd().endDebugLabel()
        end
    end

    RG->>Cmd: endRenderPass()
    RG->>Cmd: end()
    RG-->>App: 
...
sequenceDiagram
    ...
    Note over Pass,RM: 风格A: 高层便捷（一键绘制）
    Pass->>RCL: list.drawMesh(geo, pso, bindings, pushConstants, pushSize)
    RCL->>Cmd: bindPipeline(pso)
    RCL->>Cmd: bindGeometry(geo)
    RCL->>Cmd: bindUniformBuffer(slot, buf, ...)
    ...
    Note over Pass,RM: 风格B: 链式精细控制
    Pass->>RCL: list.bindPipeline(pso).bindGeometry(geo).drawIndexed(6,0,0)
    RCL->>Cmd: bindPipeline(pso)
    RCL->>Cmd: bindGeometry(geo)
    RCL->>Cmd: drawIndexed(6, 0, 0)

    Note over Pass,RM: 风格C: 直接操作底层 + 临时资源创建
    Pass->>RCL: list.cmd().beginDebugLabel("MyPass")
    RCL->>Cmd: beginDebugLabel("MyPass")
    Pass->>RCL: list.resources().createBuffer(desc, data)
    RCL->>RM: createBuffer(desc, data)
    RM-->>RCL: BufferHandle
    Pass->>RCL: list.cmd().copyBuffer(staging, dst, 0, 0, size)
    RCL->>Cmd: copyBuffer(staging, dst, 0, 0, size)
    Pass->>RCL: list.cmd().endDebugLabel()
    RCL->>Cmd: endDebugLabel()
```
...
// 底层命令录制接口
// 职责：直接翻译为 vkCmd* / gl*，是 GPU 命令的最小表达单元
// 使用者：RenderCommandList（通过高层封装后调用）
class ICommandBuffer {
public:
...
### 4.4 RenderCommandList.hpp（薄适配层）
...
// 绑定描述（drawMesh / dispatchCompute 使用）
struct Binding {
...
// 每帧的绘制适配层
// 职责：
//   1. 持有当前帧的 ICommandBuffer（从 FrameContext 分配）
//   2. 提供高层便捷方法（drawMesh = bindPipeline + bindGeometry + bindUniforms + draw）
//   3. 暴露 IRenderDevice 和 ResourceManager 引用（临时资源创建、查询）
//   4. 链式调用支持（代码紧凑）
//   5. 未来扩展：Secondary CommandBuffer 管理、自动 barrier 插入
//
// 核心原则：薄。不替代 ICommandBuffer，只是包装 + 便捷。
// 上层随时可以通过 cmd() / device() / resources() 访问底层。
//
// 生命周期：Renderer::beginFrame() 创建 → RenderGraph::execute() 使用 → Renderer::endFrame() 销毁
class RenderCommandList {
public:
    ...
    // === 高层便捷方法（一键绘制）===
    // 自动处理：bindPipeline + bindGeometry + 批量绑定 uniform/texture + pushConstants + drawIndexed
    void drawMesh(GeometryHandle geo, PipelineHandle pso,
                  const std::vector<Binding>& bindings,
                  const void* pushConstants = nullptr, uint32_t pushSize = 0);

    // 自动处理：bindPipeline + 批量绑定 storage buffer/texture + dispatchCompute
    void dispatchCompute(PipelineHandle pso,
                         const std::vector<Binding>& bindings,
                         uint32_t groupX, uint32_t groupY, uint32_t groupZ = 1);

    // === 透传 ICommandBuffer 方法（链式调用，代码紧凑）===
    // 返回 RenderCommandList& 支持链式：list.bindPipeline(pso).bindGeometry(geo).drawIndexed(6,0,0)
    RenderCommandList& bindPipeline(PipelineHandle pso);
    RenderCommandList& bindGeometry(GeometryHandle geo);
    RenderCommandList& bindUniformBuffer(uint32_t slot, BufferHandle buffer, uint64_t offset, uint64_t range);
    RenderCommandList& bindStorageBuffer(uint32_t slot, BufferHandle buffer, uint64_t offset, uint64_t range);
    RenderCommandList& bindTexture(uint32_t slot, TextureHandle texture, SamplerHandle sampler);
    RenderCommandList& bindStorageTexture(uint32_t slot, TextureHandle texture);
    RenderCommandList& pushConstants(std::span<const uint8_t> data, uint32_t offset = 0);
    RenderCommandList& drawIndexed(uint32_t indexCount, uint32_t firstIndex = 0, int32_t vertexOffset = 0);
...
| **RenderCommandList** | 透传到底层，无额外逻辑 | 禁止在 List 层做状态缓存（GL 后端自己在 ICommandBuffer 里做） |
...
| **RenderCommandList** | 透传到底层，无额外逻辑 | 禁止在 List 层做状态缓存（VK 本身无冗余绑定惩罚） |
...
9. RenderCommandList 是薄适配层，不得替代 ICommandBuffer 的职责
10. 状态缓存逻辑在各后端 ICommandBuffer 实现里做，不在 RenderCommandList 层做
```
...
│   ├── GLResourcePools.hpp / .cpp   // VAO缓存、FBO模拟、Program缓存
│   └── GLUtils.hpp / .cpp           // Format映射、VK_CHECK风格宏
├── vk/
│   ├── VKDevice.hpp / .cpp
│   ├── VKCommandBuffer.hpp / .cpp
│   ├── VKFrameContext.hpp / .cpp    // Fence/Semaphore/CmdPool Ring
│   ├── VKResourcePools.hpp / .cpp   // DescriptorPool、RenderPass缓存
│   └── VKUtils.hpp / .cpp           // Format映射、VK_CHECK宏
