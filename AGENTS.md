...
- **RHI (Rendering Hardware Interface)** (`RHI/`): New abstraction layer under `Vulkitten::rhi` namespace providing backend-agnostic rendering primitives. `RHI/Core/Types.hpp` centralizes rendering types (Extent2D/3D, Viewport, ClearValue, ShaderStage, BufferUsage, TextureUsage, MemoryProperty, PipelineStage, AccessFlags, ImageLayout, FilterMode/WrapMode, etc.). `RHI/Core/Format.hpp` provides cross-API `Format` enum (R8_UNORM..BC7_RGBA_UNORM, D16_UNORM..D32_FLOAT_S8_UINT) with `FormatByteSize`/`FormatComponentCount`/`IsDepthFormat`/`IsStencilFormat` utilities. Additional RHI headers (Handle, PipelineDesc) will be added in Tasks 3-4. All types are pure C++17 — no Vulkan or OpenGL types in RHI headers.
