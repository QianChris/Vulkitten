...
- **RHI (Rendering Hardware Interface)** (`RHI/`): New abstraction layer under `Vulkitten::rhi` namespace providing backend-agnostic rendering primitives. `RHI/Core/Types.hpp` centralizes rendering types (Extent2D/3D, Viewport, ClearValue, ShaderStage, BufferUsage/TextureUsage/MemoryProperty, PipelineStage/AccessFlags/ImageLayout, FilterMode/WrapMode/LoadOp/StoreOp). `RHI/Core/Format.hpp` provides cross-API `Format` enum (R8_UNORM..BC7_RGBA_UNORM, D16_UNORM..D32_FLOAT_S8_UINT) with utility functions. `RHI/Handle.hpp` provides `Handle<Tag>` (uint32_t id + generation) with strong Tag-based type safety — Tag structs prevent mixing BufferHandle/TextureHandle/etc. at compile time; generation counter protects against use-after-free (ABA) on slot reuse. `RHI/RHIPipelineDesc.hpp` will be added in Task 4. All RHI types are pure C++17 — no Vulkan or OpenGL types in RHI headers.
